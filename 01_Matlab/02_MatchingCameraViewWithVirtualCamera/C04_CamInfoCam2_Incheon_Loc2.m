...
close all;
% Load 3D field data
load("D02_3DFieldData_rot2.mat");

% Read image
parentFolder = 'G:\My Drive\Work\_GIT (2023~)\01 Mudskipper\20241216 3DTrackVideoCases\01_Videos\Incheon_Loc2\Vd02'; % Change this to your parent folder path
...
imagePoint2D = [1152; 618; 1]; % Homogeneous 2D point
...
cameraPosition = [-0.770627868179118,0.355167941962867,0.511076041211644];  % Camera location in 3D space
cameraTarget = [-0.42,0.888522375571166,0.5]; % Point the camera is looking at
...
focalLengthX = 1300; %850;  %1351.4236; 
...
radialDistortion = [0,0];%[-0.07, -0.05];%[-0.02, 0];  % No radial distortion (from the provided data)
...
validX = imagePoints(1, :) >= 0 & imagePoints(1, :) <= imageWidth;
validY = imagePoints(2, :) >= 0 & imagePoints(2, :) <= imageHeight;
...
imageData = imresize(imageData, [imageHeight, imageWidth]);
figure(1); 
...
% Customize the grid spacing (optional)
ax.XTick = ax.XLim(1):2*lineSpacing:ax.XLim(2); % Set X-axis ticks at 100px intervals
ax.YTick = ax.YLim(1):lineSpacing:ax.YLim(2); % Set Y-axis ticks at 100px intervals
...
save('camera_parameters_Cam2_Incheon_Loc2.mat', 'cameraPosition', 'cameraTarget', 'cameraUpVector', 'imageWidth', 'imageHeight', ...
